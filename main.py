...
from branding import APP_NAME, APP_SHORT_NAME, COMPANY_NAME, PRODUCT_NAME
from oneboard_log_safety import RedactingFormatter, safe_exception_text
...
def setup_logging():
    ...
    def _excepthook(exc_type, exc_value, exc_tb):
        ...
        # The configured formatter already emits a redacted traceback. Calling
        # the original hook would duplicate the raw exception on stderr, which
        # the packaged launcher captures in launcher.log.
...
def create_app_icon() -> QIcon:
    ...
    # Draw the OB monogram as paths so the application icon is independent of
    # fonts installed on the customer machine.
    p.setBrush(Qt.BrushStyle.NoBrush)
    p.setPen(QPen(QColor(255, 255, 255), 4, Qt.PenStyle.SolidLine,
                  Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin))
    p.drawEllipse(8, 19, 18, 26)
    monogram = QPainterPath()
    monogram.moveTo(34, 19)
    monogram.lineTo(34, 45)
    monogram.moveTo(34, 20)
    monogram.lineTo(42, 20)
    monogram.cubicTo(51, 20, 51, 32, 42, 32)
    monogram.lineTo(34, 32)
    monogram.moveTo(42, 32)
    monogram.cubicTo(52, 32, 52, 45, 42, 45)
    monogram.lineTo(34, 45)
    p.drawPath(monogram)
    p.end()
    return QIcon(pix)
...
class LiveTranslateApp:
    def __init__(self, config):
        ...
        self._transcript = TranscriptWriter(data_dir() / "transcripts")
    ...
    def _switch_asr_engine(self, engine_type: str):
        ...
        def _load():
            ...
            try:
                ...
                log.error(
                    f"Failed to load ASR worker: {safe_error}",
                ...
                if old_config:
                    try:
                        ...
                        safe_restore_error = safe_exception_text(restore_exc)
                        restore_error[0] = safe_restore_error
                        log.error(
                            "Failed to restore previous ASR worker: "
                            f"{safe_restore_error}",
                            exc_info=True,
                        )
    ...
    def _check_memory_threshold(self, rss_mb: float):
        ...
        log.warning(
            ...
            f"Recommend restarting {APP_NAME} to free C-side allocator caches."
        )
    ...
    def _translate_async(self, msg_id, text, source_lang, extra_langs=None):
        ...
        try:
            ...
            if isinstance(e, (openai.APIConnectionError, openai.APITimeoutError,
                ...
                log.warning(f"Translate error: {safe_error}")
            else:
                log.error(f"Translate error: {safe_error}", exc_info=True)
            self._transcript.finalize_no_translation(msg_id)
            if self._overlay:
                self._overlay.update_translation(msg_id, f"[error: {safe_error}]", 0)
        ...
        for future in as_completed(futures):
            try:
                ...
                if isinstance(e, (openai.APIConnectionError, openai.APITimeoutError,
                                  openai.AuthenticationError, openai.APIStatusError,
                                  TimeoutError, ConnectionError)):
                    log.warning(f"Extra translate error: {safe_error}")
                else:
                    log.error(f"Extra translate error: {safe_error}", exc_info=True)
...
def main():
    # Keep the upstream orchestration available for development, while the
    # customer entry point uses the isolated OneBoard product layer.
    from oneboard_app import run
    run()
...
if __name__ == "__main__":
    ...
    # oneboard_app imports the pipeline classes from ``main``. When this file is
    # launched as a script, publish the current module under that name so Python
    # does not execute this large module a second time.

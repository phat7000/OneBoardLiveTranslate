...
from oneboard_log_safety import (
    RedactingFormatter,
...
class _StderrCapture:
    ...
    def write(self, text):
        ...
        while True:
            boundaries = [
                index
                for index in (self._buffer.find("\r"), self._buffer.find("\n"))
                if index >= 0
            ]
            if not boundaries:
                break
            ...
            if self._buffer[boundary : boundary + 2] == "\r\n":
                end += 1
            complete = self._buffer[:end]
            self._buffer = self._buffer[end:]
            self._emit(complete)
        return len(text)

    def _emit(self, text):
        # Normalize terminal control sequences before matching so ANSI codes
        # cannot split a credential label or token around the redactor.
    ...
    def flush(self):
        # Keep an incomplete logical line buffered so credentials split across
        # several write/flush calls can still be redacted as one record.
        if self._orig:
            self._orig.flush()

    def finish(self):
        """Redact and forward any final unterminated record before restoring."""
        if self._buffer:
...
class SetupWizardDialog(QDialog):
    ...
    def _start_download(self):
        ...
        self._stderr_capture = _StderrCapture(
            self._log_signal.emit, self._orig_stderr
        )
    ...
    def _download_worker(self, hub, proxy):
        try:
            ...
            self._error = safe_exception_text(e)
            log.error(f"Download failed: {self._error}", exc_info=True)
